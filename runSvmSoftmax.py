...
################################################################################
# regularly pick up the learning rates, the former one times 3 equals to the latter one
learningRates = [0.001,0.003,0.01,0.03,0.1]
# randomly pick up the regularization coefficient
regularizationCoefficients = [0,0.008,0.01,0.02,0.05]
# run the model under every circumstance
for lr in learningRates:
    for rr in regularizationCoefficients:
        model = Svm(xTrain.shape[1],numClasses)
        model.train(xTrain,yTrain,lr,rr,iter = 100,verbose = False)
        acc = model.calAccuracy(xVal,yVal)
        if(acc > bestAcc):
            bestAcc = acc
            bestModel = model
        bestParameters = [lr,rr]
pass
################################################################################
#                              END OF YOUR CODE                                #
################################################################################
...
for lr in learningRates:
    for rr in regularizationCoefficients:
        model = Softmax(xTrain.shape[1],numClasses)
        model.train(xTrain,yTrain,lr,rr,iter = 100,verbose = False)
        acc = model.calAccuracy(xVal,yVal)
        if(acc > bestAcc):
            bestAcc = acc
